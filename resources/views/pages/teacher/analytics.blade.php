...
@php
    ...
    $classChartWidth = max(560, max(1, $scoreByClass->count()) * 86);
    $weeklyChartWidth = 560;
    $activityChartWidth = max(560, max(1, count($activityTrend)) * 44);
    $formatPercent = fn ($value, $precision = 1) => $value !== null ? number_format($value, $precision) . '%' : '—';
@endphp

@push('styles')
<style>
.analytics-page { display:grid; gap:1.25rem; min-width:0; }
.analytics-header { display:grid; grid-template-columns:minmax(0, 1fr) minmax(24rem, 42rem); gap:1.5rem; align-items:start; }
.analytics-heading h1 { margin:0 0 .35rem; }
.analytics-heading p { margin:0; color:var(--muted-foreground); }
...
.period-tab { border:0; border-radius:calc(var(--radius-md) - 2px); padding:.45rem .8rem; background:transparent; color:var(--muted-foreground); font-size:var(--text-sm); font-weight:700; cursor:pointer; text-align:center; text-decoration:none; }
...
.analytics-grid { display:grid; gap:1.25rem; min-width:0; }
.grid-2 { grid-template-columns:repeat(2, minmax(0, 1fr)); }
.grid-3 { grid-template-columns:minmax(0, 2.4fr) minmax(18rem, .9fr); }
.analytics-panel { border:1px solid var(--border); border-radius:var(--radius-lg); background:var(--card); padding:1.25rem; box-shadow:0 1px 2px rgba(15,23,42,.03); overflow:hidden; min-width:0; }
.panel-head { display:flex; align-items:flex-start; justify-content:space-between; gap:1rem; margin-bottom:1rem; }
.panel-title { font-size:var(--text-lg); font-weight:800; margin:0; letter-spacing:-.02em; }
.panel-note { color:var(--muted-foreground); font-size:var(--text-sm); margin-top:.2rem; line-height:1.45; }
...
.chart-scroll { width:100%; max-width:100%; overflow-x:auto; overflow-y:hidden; padding-bottom:.35rem; }
.chart-scroll::-webkit-scrollbar, .table-wrap::-webkit-scrollbar { height:.5rem; }
.chart-scroll::-webkit-scrollbar-thumb, .table-wrap::-webkit-scrollbar-thumb { background:color-mix(in srgb, var(--muted-foreground) 24%, transparent); border-radius:999px; }
.bar-chart { display:flex; align-items:flex-end; gap:.65rem; height:220px; padding:.5rem .15rem 0; }
.bar-col { flex:0 0 66px; height:100%; display:flex; flex-direction:column; align-items:center; gap:.35rem; min-width:0; }
...
.bar-value { font-size:var(--text-xs); font-weight:800; color:var(--primary); min-height:1rem; }
...
.donut-number { font-size:var(--text-2xl); font-weight:900; line-height:1; }
...
.trend-bars { display:flex; align-items:flex-end; gap:.55rem; height:178px; padding:.25rem .15rem 0; }
.trend-col { flex:0 0 36px; height:100%; display:flex; flex-direction:column; align-items:center; gap:.3rem; min-width:0; }
.trend-track { flex:1; width:100%; display:flex; align-items:flex-end; justify-content:center; border-bottom:1px solid var(--border); }
.trend-fill { width:100%; max-width:2.5rem; min-height:.25rem; border-radius:var(--radius-sm) var(--radius-sm) 0 0; background:var(--primary); opacity:.86; }
.activity-fill { background:#0f766e; }
.trend-value { min-height:1rem; font-size:var(--text-xs); font-weight:800; color:var(--primary); }
.trend-label { font-size:var(--text-xs); color:var(--muted-foreground); text-align:center; white-space:nowrap; }
.rank-list, .activity-list { display:grid; gap:.15rem; }
.rank-row, .activity-row { display:flex; align-items:center; gap:.75rem; padding:.7rem 0; border-top:1px solid var(--border); min-width:0; }
.rank-row:first-child, .activity-row:first-child { border-top:0; }
.rank-pos { width:1.75rem; text-align:center; font-weight:900; color:var(--muted-foreground); }
.avatar-chip { width:2.25rem; height:2.25rem; border-radius:999px; display:flex; align-items:center; justify-content:center; flex:0 0 auto; font-size:var(--text-xs); font-weight:900; background:color-mix(in srgb, var(--primary) 12%, transparent); color:var(--primary); }
.row-main { flex:1; min-width:0; }
.row-title { font-size:var(--text-sm); font-weight:700; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
...
.score-pill { border-radius:999px; padding:.2rem .55rem; font-size:var(--text-xs); font-weight:900; background:color-mix(in srgb, var(--primary) 10%, transparent); color:var(--primary); white-space:nowrap; }
.status-pill { border-radius:999px; padding:.2rem .55rem; font-size:var(--text-xs); font-weight:800; background:var(--muted); color:var(--muted-foreground); white-space:nowrap; }
...
.analytics-table th { color:var(--muted-foreground); font-size:var(--text-xs); text-transform:uppercase; letter-spacing:0; font-weight:900; }
...
@media (max-width: 640px) {
    ...
    .legend-row strong { grid-column:2 / 4; }
}
</style>
@endpush
...
<div class="analytics-page">
    ...
    <div class="stats-grid stats-grid-4 stagger-children">
        <div class="stat-card analytics-stat">
            ...
            <div class="stat-card__value {{ $summary['avg_score'] === null ? 'muted-value' : '' }}">{{ $formatPercent($summary['avg_score']) }}</div>
        ...
        <div class="stat-card analytics-stat">
            ...
            <div class="stat-card__value {{ $summary['completion_rate'] === null ? 'muted-value' : '' }}" style="{{ $summary['completion_rate'] !== null ? 'color:var(--success);' : '' }}">{{ $formatPercent($summary['completion_rate']) }}</div>
        ...
        <section class="analytics-panel">
            ...
            @if($scoreByClass->whereNotNull('avg_score')->isNotEmpty())
                <div class="chart-scroll">
                    <div class="bar-chart" style="width:{{ $classChartWidth }}px;">
    ...
    <div class="analytics-grid grid-2 stagger-children">
        <section class="analytics-panel">
            ...
            @if(collect($weeklyTrend)->sum('count') > 0)
                <div class="chart-scroll">
                    <div class="trend-bars" style="width:{{ $weeklyChartWidth }}px;">
        ...
        <section class="analytics-panel">
            ...
            @if(collect($activityTrend)->sum('count') > 0)
                <div class="chart-scroll">
                    <div class="trend-bars" style="width:{{ $activityChartWidth }}px;">
    ...
    <div class="analytics-grid grid-3 stagger-children">
        <section class="analytics-panel">
            <div class="panel-head">
                <div>
                        ...
                        <tbody>
                            @foreach($scoreByClass as $item)
                                <tr>
                                    ...
                                    <td class="num">{{ $formatPercent($item['avg_score']) }}</td>
                                    <td>
                                        <div style="display:flex;align-items:center;gap:.6rem;">
                                            <div class="progress-line"><span style="width:{{ $item['completion_rate'] ?? 0 }}%;"></span></div>
                                            <strong>{{ $formatPercent($item['completion_rate'], 0) }}</strong>
                                        </div>
                                    </td>
