...
@push('styles')
<style>
.result-hero { background:linear-gradient(135deg,#eff6ff,#fff7ed); padding:3rem 1.5rem; text-align:center; }
    .dark .result-hero { background:linear-gradient(135deg,#1e2235,#1a1d2e); }
    .score-circle { width:140px; height:140px; border-radius:50%; display:flex; flex-direction:column; align-items:center; justify-content:center; margin:0 auto 1.5rem; font-weight:700; }
...
@section('content')
  <!-- Result hero -->
      <div class="result-hero animate-fade-in">
        <div class="score-circle" id="score-circle" style="background:color-mix(in srgb,var(--success) 15%,transparent); color:var(--success);">
          <span style="font-size:2.5rem;" id="score-pct">80%</span>
          <span style="font-size:var(--text-sm);font-weight:500;" id="score-grade">Xếp loại B</span>
        </div>
        <h1 id="result-title">Chúc mừng! Bạn đã đạt</h1>
        <p style="color:var(--muted-foreground);" id="quiz-name">React Hooks Nâng cao</p>
        <div style="display:flex;justify-content:center;gap:2.5rem;margin:1.5rem 0;">
          <div class="stat-mini"><div class="stat-mini-val" id="correct-count" style="color:var(--success);">8</div><div class="stat-mini-label">Câu đúng</div></div>
          <div class="stat-mini"><div class="stat-mini-val" id="wrong-count"   style="color:var(--destructive);">2</div><div class="stat-mini-label">Câu sai</div></div>
          <div class="stat-mini"><div class="stat-mini-val" id="skipped-count" style="color:var(--muted-foreground);">0</div><div class="stat-mini-label">Bỏ qua</div></div>
          <div class="stat-mini"><div class="stat-mini-val" id="time-spent">38:22</div><div class="stat-mini-label">Thời gian làm</div></div>
        </div>
        <div style="display:flex;justify-content:center;flex-wrap:wrap;gap:0.75rem;">
          ...
          <a href="{{ route('student.quiz-take') }}" class="btn btn-outline gap-2">Làm lại</a>
          <button class="btn btn-primary gap-2" onclick="window.print()">Xem lại đáp án</button>
        </div>
      </div>
      ...
      <div style="padding:1.5rem;">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;" class="stagger-children">
          <!-- Chart -->
          <div class="card">
            <div class="card-header"><h3 class="card-title">Phân tích theo chủ đề</h3><p class="card-description">Kết quả chi tiết từng phần kiến thức</p></div>
            <div class="card-content"><canvas id="breakdownChart" height="220"></canvas></div>
          </div>
          <!-- Achievements -->
          <div class="card">
            <div class="card-header"><h3 class="card-title">Thành tích đạt được</h3></div>
            <div class="card-content">
              <div style="display:flex;flex-direction:column;gap:0.75rem;">
                <div class="flex items-center gap-0.75" style="gap:0.75rem;padding:0.875rem;background:color-mix(in srgb,var(--warning) 10%,transparent);border-radius:var(--radius-md);border:1px solid color-mix(in srgb,var(--warning) 25%,transparent);">
                  <span style="font-size:1.5rem;">🏆</span>
                  <div><div style="font-weight:600;">Top 10%</div><div style="font-size:var(--text-xs);color:var(--muted-foreground);">Nằm trong nhóm 10% điểm cao nhất</div></div>
                </div>
                <div class="flex items-center gap-0.75" style="gap:0.75rem;padding:0.875rem;background:color-mix(in srgb,var(--primary) 10%,transparent);border-radius:var(--radius-md);border:1px solid color-mix(in srgb,var(--primary) 25%,transparent);">
                  <span style="font-size:1.5rem;">⚡</span>
                  <div><div style="font-weight:600;">Học Nhanh</div><div style="font-size:var(--text-xs);color:var(--muted-foreground);">Hoàn thành trong 85% thời gian quy định</div></div>
                </div>
                <div class="flex items-center gap-0.75" style="gap:0.75rem;padding:0.875rem;background:color-mix(in srgb,var(--success) 10%,transparent);border-radius:var(--radius-md);border:1px solid color-mix(in srgb,var(--success) 25%,transparent);">
                  <span style="font-size:1.5rem;">✅</span>
                  <div><div style="font-weight:600;">Vượt qua bài kiểm tra</div><div style="font-size:var(--text-xs);color:var(--muted-foreground);">Đạt điểm chuẩn yêu cầu</div></div>
                </div>
              </div>
            </div>
          </div>
